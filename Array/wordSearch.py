...
#DFS search, use one additional stack to save tmp str values and 
#set it back when poping elements
...
class Solution:
    ...
    def dfs(self, board, row, column, checkingIndex, word):
        ...
        #When the node we get is a backtrack, we known we visited all its children
    ...
    def exist(self, board: 'List[List[str]]', word: 'str') -> 'bool':
        ...
        for row in range(0, len(board)):
            for column in range(0, len(board[0])):
                if board[row][column] == word[0]:
                    #pass currentChecking Index at word
                    if self.dfs(board, word, 0, row, column):
                        return True
        return False
    def dfs(self, board, word, index, currentRow, currentColumn):
        if index >= len(word):
            return True
        if word[index] == board[currentRow][currentColumn]:
            if index == len(word) - 1:
                return True
            tmp = board[currentRow][currentColumn]
            board[currentRow][currentColumn] = "#"
            for row, column in ((currentRow, currentColumn - 1), (currentRow, currentColumn + 1), (currentRow -1 , currentColumn), (currentRow +1 , currentColumn)):
                if (row < len(board) and row >= 0) and (column < len(board[0]) and column >= 0):
                    if board[row][column] != "#":
                        if self.dfs(board, word, index + 1, row, column):
                            return True
            board[currentRow][currentColumn] = tmp
        return False        
